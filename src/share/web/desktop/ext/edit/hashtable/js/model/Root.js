js.extend('lsn.hashtable.model', function (js) {
  ...
  Root.onFetchNewNode = function (result, name) {
    if (this.hasChildNodes()) {
      ...
      this.autoselect = dnode;
    }
  };

  Root.onAdopt = function (node) {
    if (this.autoselect && this.autoselect === node.getDataNode()) {
      node.setAttribute('autoselect', true);
      this.autoselect = null;
    }
    this.dispatchAction('create', node);
  };

  Root.onOrphan = function (node) {
    this.dispatchAction('remove', node);
  };
