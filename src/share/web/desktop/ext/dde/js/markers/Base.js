ECMAScript.Extend('lsn.ext.dde.markers', function (ecma) {
  ...
  this.Base = function (dde, pelem) {
    this.dde = dde;
    this.attrs = null;
    this.menu = new ecma.lsn.ext.dde.PopupMenu(dde);
  ...
  var Base = this.Base.prototype = ecma.lang.createMethods();

  Base.getBoundingBox = ecma.lang.createAbstractFunction();

  Base.getType = function () {
    return 'unknown';
  };

  Base.getEditor = ecma.lang.createAbstractFunction();
 ...
 * ContentMarker.js) dialog.
  ...
  Base._onClick = function (event) {
    ecma.dom.setTimeout(this.doClick, 250, this, [event]);
  };
...
*/
  ...
  Base.refresh = function () {
    ...
    for (var i = 0, img; img = this.images[i]; i++) {
      var l = this.box.left + img.offsetLeft + this.dde.body_x + this.adjust.x;
      var t = this.box.top + img.offsetTop + this.dde.body_y + this.adjust.y;
      l = ecma.util.asInt(l, true);
      t = ecma.util.asInt(t, true);
