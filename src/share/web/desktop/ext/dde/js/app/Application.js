ECMAScript.Extend('lsn.ext.dde', function (ecma) {
  ...
  var _cookies = new ecma.http.Cookies();
  var proto = ecma.lang.createMethods(CActionDispatcher);

  this.Application = function (menuId, sbId, frameId) {
    CActionDispatcher.apply(this);
    this.saving = null;
    this.baseLoc = new ecma.http.Location();
    this.docLoc = undefined;
    this.menuId = menuId;
    this.frameId = frameId;
    this.dc = new ecma.hubb.getInstance();
    this.editor = undefined;
    this.marker = undefined;
    this.js = undefined;
    this.felem = ecma.dom.getElement(frameId);
    this.frame = ecma.dom.getFrame(frameId);
    this.status = new ecma.lsn.ext.dde.StatusBar(sbId);
    this.deltas = null;
    this.markers = [];
    this.masks = [];
    this.editors = {};
    this.mm = new ecma.lsn.ext.dde.MenuManager(this, this.menuId);
    this.reloadAfterCommit = null;
    this.vLoad = new ecma.dom.EventListener(this.felem, 'load', this.onFrameLoad, this);
    this.vBeforeUnload = new ecma.dom.EventListener(ecma.window, 'beforeunload', this.onBeforeUnload, this);
    this.attachToWindow();
  };
  ...
  proto.attachToWindow = function () {
    this.js = ecma.dom.getContentJS(this.felem);
    this.js.dom.addEventListener(this.js.window, 'load', function (event) {
      ecma.console.log('Child window onLoad event.');
    }, this);
  ...
  proto.getTargetJS = function () {
    try {
      var doc = ecma.dom.getContentDocument(this.frame);
      var win = ecma.dom.getContentWindow(this.frame);
      var tjs = new ECMAScript.Class(win, doc);
      if (!ecma.http.isSameOrigin(ecma.document.location.href, doc.location.href)) {
        // For unsecure browsers which do not raise an exception
        tjs = null;
      }
      return tjs;
    } catch (ex) {
  ...
  proto.onFrameLoad = function (event) {
    ...
    if (this.saving) {
      if (docLoc && docLoc.isSameDocument(this.saving)) {
        ...
        ecma.dom.setTimeout(this.onFrameLoad, 250, this, [event]);
      }
      return;
    }
    this.editors = {};
    ...
    this.ui = {
      popups: this.js.dom.createElement('div', {
        id:'dde-popup-layer',
        style:{'position':'absolute', 'z-index': 10000, 'top':0, 'left':0}
      }),
    ...
    if (this.js.dom.getStyle(this.body, 'position') == 'relative') {
      this.body_x = ecma.util.asInt(this.js.dom.getStyle(this.body, 'margin-top'));
      this.body_y = ecma.util.asInt(this.js.dom.getStyle(this.body, 'margin-left'));
    } else {
      this.body_x = 0;
      this.body_y = 0;
    }
    //ecma.console.log('body adjust', this.body_x, this.body_y);
...
//  this.js.dom.addEventListener(this.js.window, 'beforeunload', this.onFrameBeforeUnload, this);
    ...
    this.doUnload = true;
    this.init();
    this.deltas = new ecma.lsn.ext.dde.Deltas(this.dc);
  ...
  proto.onCommitComplete = function () {
    try {
      ...
      if (this.reloadAfterCommit) {
        ...
        ecma.lsn.hideMask();
        new ecma.http.Request(this.saving).submit();
      }
    } finally {
      this.saving = null;
    }
  ...
  proto.init = function () {
    ...
    for (var i = 0, elem; elem = elems[i]; i++) {
      this.markers.push(this.createMarker(elem));
    }
    this.adjustMarkers();
  };
  ...
  proto.makeEditableElements = function (pelem, result) {
    var comments = this.js.dom.getElementsByNodeType(pelem,
      ecma.dom.constants.COMMENT_NODE);
    var blocks = [];
    var block = null;
    var spec = null;
    for (var i = 0, cnode; cnode = comments[i]; i++) {
      ...
      if (spec) {
        ...
        block = new ecma.lsn.ext.dde.BlockInfo();
  ...
  proto.getEditableElements = function (pelem, result) {
    for (var i = 0, node = null, nodes = pelem.childNodes; node = nodes[i]; i++) {
      ...
      var ds = ecma.dom.getAttribute(node, '_lsn_ds');
  ...
  proto.createMarker = function (target) {
    if (ecma.util.isa(target, ecma.data.Node)) {
      return new ecma.lsn.ext.dde.markers.BlockMarker(this, target);
    } else {
      return new ecma.lsn.ext.dde.markers.ContentMarker(this, target);
    }
  };

  // Adjust marker's position where it is affected by a floating element
  ...
  proto.cmds = {
    ...
    docOpen: function () {
      if (!this.dlgOpen) {
        this.dlgOpen = new ecma.hubb.ui.BrowseDialog();
        this.dlgOpen.dlg.addEvent('ok', ecma.lang.Callback(function () {
    ...
    docSave: function (reload) {
      ...
      ecma.lsn.showMask();
      ecma.dom.setTimeout(this.deltas.commit, 0, this.deltas);
    },

    docGoto: function () {
      ecma.window.open(this.js.window.location.href);
    },
    ...
    docViewSource: function () {
      ...
      if (!this.dlgViewSource) {
        this.dlgViewSource = new ecma.lsn.Dialog('[#`./dlg-view-source.html`]', {'refetch':false});
      }
    ...
    docMail: function () {
      if (!this.dlgMail) {
        this.dlgMail = new ecma.lsn.Dialog('[#`./dlg-mail.html`]', {'refetch':false});
        this.dlgMail.addEvent('mail-sent', ecma.lang.Callback(function () {
  ...
  proto.canUnload = function () {
    ...
    return !this.deltas.isUpToDate() ? confirm('Discard changes?') : true;
  };
  ...
  proto.hideMarkers = function () {
    ecma.dom.setStyle(this.ui.markers, 'display', 'none');
  };
  ...
  proto.hideMask = function () {
    ecma.dom.setStyle(this.ui.masks, 'display', 'none');
    if (this.maskType) {
      ecma.dom.removeClassName(this.ui.masks, this.maskType);
      this.maskType = null;
    }
    this.marker = null;
  };
  ...
  proto.setMaskType = function (type) {
    this.maskType = type;
    ecma.dom.addClassName(this.ui.masks, this.maskType);
  };
  ...
  proto.select = function (marker, cmd) {
    ...
    ecma.dom.addClassName(this.ui.masks, 'shaded');
  ...
  proto.deselect = function () {
    ...
    this.marker = null;
    this.hideMask();
    ecma.dom.removeClassName(this.ui.masks, 'shaded');
    this.showMarkers();
